...
int main()
{
    ...
    for (int i = 0; i < n_spheres; i++)
    {
        ...
        float y_pos = 2.0f + (i * 2.0f);
        sphere.rb.transform.position = linkit::Vector3(x_pos, y_pos, z_pos);
        // Give spheres some initial velocity for movement
        sphere.rb.velocity = linkit::Vector3((i % 2 == 0 ? 2.0f : -2.0f), 0, (i % 3 == 0 ? 1.5f : -1.5f));
        scene.add_game_object(sphere);

        // Apply gravity to each sphere (index offset by 5 for the 5 box pieces: floor + 4 walls)
        scene.force_registry.add(&scene.game_objects[5 + i], std::make_shared<SimpleGravity>(gravity));
    }
